SimpleILIASDashboard = (function() {
  ...
  pub.createPHPUnitWidget = function(
      url, version, job_id, id, title, warn, skip, incomp, risky, fail, failure,
      date, timestamp) {
    let failed = '',
        formatted_date = timestamp !== undefined ? new Intl.DateTimeFormat(
            'default', {
              weekday: 'long',
              year:    'numeric',
              month:   'long',
    ...
    return '<a href="' + url + '" class="col-xl-4 col-lg-5" target="_blank"> ' +
        ...
        pri.html_snippets.phpunit_state_html + ' text-warnings' + failed +
        '"></i> ' + warn + ' Warnings </p>' +
        pri.html_snippets.phpunit_state_html + ' text-skipped' + failed +
        '"></i> ' + skip + ' Skipped </p>' +
        pri.html_snippets.phpunit_state_html + ' text-incomplete' + failed +
        '"></i> ' + incomp + ' Incomplete </p>' +
        pri.html_snippets.phpunit_state_html + ' custom-risky' + failed +
        '"></i> ' + risky + ' Risky </p>' +
        pri.html_snippets.phpunit_state_html + ' text-failed' + failed +
        '"></i> ' + fail + ' Failed </p>' +
        ' </div>' +
        ' </div>' +
        ' </div>' +
  ...
  pub.createDictoWidget = function(
      date, job_id, url, jf, total, resolved, added) {

    if (jf > 0) {
      jf = '+' + jf;
    }

    let title = '<h6>Dicto ' + date + '</h6>' + '<a href=" ' + url + ' "><h6>' +
        job_id + '</h6></a>';
    let values = pri.html_snippets.dicto_state_html +
        ' badge-warning mr-2" href="#">' + total + ' Total</a> </span>' +
        pri.html_snippets.dicto_state_html + ' badge-success mr-2" href="#">' +
        resolved + ' Resolved</a> </span>' +
        pri.html_snippets.dicto_state_html + ' badge-danger mr-2" href="#">' +
        added + ' Added</a> </span>';

    if (jf !== '0') {
      title = 'Since last Jour fixe';
      values = pri.html_snippets.dicto_state_html +
          ' badge-warning mr-2" href="#">' + jf + ' Total</a> </span>' +
          pri.html_snippets.dicto_state_html +
          ' badge-success mr-2" href="#">' + resolved +
          ' Resolved</a> </span>' +
          pri.html_snippets.dicto_state_html + ' badge-danger mr-2" href="#">' +
          added + ' Added</a> </span>';
    }
  ...
  pub.initialiseGraph = function(
      card_id, card_object, failure, warn, skip, incomp, risky, failed,
      complete) {
    ...
    if (warn == 0 && skip == 0 && incomp == 0 && risky == 0 && failed == 0) {
      middle = '{ labels: ["Complete"], datasets: [ {data: [ ' + complete +
          '], backgroundColor: ["#36d64e"], hoverBackgroundColor: ["#ffa500"],hoverBorderColor: "rgba(234, 236, 244, 1)"}],';
    }
    let tail = '},' +
        'options: {' +
        'elements: ' +
        ...
        'center: {' +
        'text: "Tests: ' + complete +
        '",color: "#212529", fontStyle: "Helvetica", sidePadding: 20 ' +
        '}' +
        '}, ' +
  ...
  pub.replaceLoaderSymbolForPHPUnitCard = function(
      card_id, failure, warn, skip, incomp, risky, failed, complete) {
    ...
    pub.initialiseGraph(card_id, card_object, failure, warn, skip, incomp,
        risky, failed, complete);
  };
    ...
    for (let singleRow = 0; singleRow < allRows.length; singleRow++) {
      ...
      if (cells.length > 1) {
        ...
        if ($('.phpunit_data').find('.' + version_string).length === 0) {
          ...
          $('.phpunit_data').
              append('<div class="' + version_string +
                  ' col-md-12 phpunit_refresh"><h5>' + version_readable +
                  '</h5></div>');
        }
        $('.phpunit_data .' + version_string).
            append(
                pub.createPHPUnitWidget(url, version_string, job_id, id, title,
                    warn, skip, incomp, risky, failed, failure, date,
                    timestamp));

        let interval = setInterval(function() {
          SimpleILIASDashboard.replaceLoaderSymbolForPHPUnitCard(
              version_string + '_' + id + '_card', failure, warn, skip, incomp,
              risky, failed, complete);
  ...
  pub.createDictoWidgets = function(data) {
    ...
    for (let singleRow = 0; singleRow < allRows.length; singleRow++) {
      ...
      if (cells.length > 1) {
        ...
        $('.dicto-data').
            append(pub.createDictoWidget(date, job_id, url, jf, total, resolved,
                added));
      }
    }
  };
  ...
  pub.startPHPUnitRefreshTimer = function() {
    pri.phpunit_refresh_handle = setInterval(function() {
  ...
  pub.appendSwitcherEvent = function() {
    ...
    $('.phpunit_toogle').on('mousedown', function() {
      if (pri.phpunit_refresh === false) {
        ...
        clearInterval(pri.phpunit_refresh_handle);
